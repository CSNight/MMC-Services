...
class Cast5Encrypt:
    def __init__(self):
        ...
        self.tempFileExt = ".cast"
        self.__iteration_count = 10000
        self.__salt_size = 16
        self._backend = default_backend()

    def cast_byte_encrypt(self, input_byte, encrypt_key):
        padder = padding.PKCS7(CAST5.block_size).padder()
        ...
        encryptor = Cipher(
            CAST5(pbk_key), modes.CBC(self.__IV[0:8]), self._backend
        ).encryptor()
        cipher_text = encryptor.update(padded_data) + encryptor.finalize()
        return cipher_text

    def cast_byte_decrypt(self, input_byte, encrypt_key):
        pbk = PBKDF2HMAC(hashes.SHA256, self.__salt_size, self.__salt, self.__iteration_count, self._backend)
        pbk_key = pbk.derive(encrypt_key.encode())
        decryptor = Cipher(
            CAST5(pbk_key), modes.CBC(self.__IV[0:8]), self._backend
        ...
        un_padder = padding.PKCS7(CAST5.block_size).unpadder()
        un_padded_data = un_padder.update(cipher_text) + un_padder.finalize()
        return un_padded_data

    def cast_str_encrypt(self, input_str, encrypt_key):
        return base64.b64encode(self.cast_byte_encrypt(input_str.encode(), encrypt_key)).decode()

    def cast_str_decrypt(self, input_str, decrypt_key):
        return self.cast_byte_decrypt(base64.b64decode(input_str), decrypt_key).decode()

    def cast_file_encrypt(self, file_path, encrypt_key, encrypt_file=None):
            ...
            with open(encrypt_file, "ab+") as new_f:
                if old_f.readable():
                    ...
                    for i in range(block_count):
                        ...
                        encrypt_chunk = self.cast_byte_encrypt(chunk, encrypt_key)
    ...
    def cast_file_decrypt(self, encrypt_file, encrypt_key, decrypt_file):
            ...
            with open(decrypt_file, "ab+") as new_f:
                if old_f.readable():
                    ...
                    for i in range(block_count):
                        ...
                        chunk = old_f.read(block_size)
                        encrypt_chunk = self.cast_byte_decrypt(chunk, encrypt_key)
                        new_f.write(encrypt_chunk)
                        new_f.flush()
                        del chunk, encrypt_chunk, block_size
                        sys.stdout.write(
